...
{
    private readonly WorkQueue[] queues;
    private readonly Thread[] workers;
    private readonly SemaphoreSlim workAvailable = new(0);
    ...
    public WorkStealingJobSystem(int workerCount)
    {
        if (workerCount <= 0)
            throw new ArgumentOutOfRangeException(nameof(workerCount));

        this.workerCount = workerCount;
        queues = new WorkQueue[workerCount];
    ...
    private void QueueSegments(JobState state, JobExecuteDelegate execute, int iterationCount, JobScheduleOptions options)
    {
        ...
        for (var segmentIndex = 0; segmentIndex < segments; segmentIndex++)
        {
            ...
            {
                ExecuteSegment(state, execute, start, count, token, inline: true);
            }
            else
            {
                EnqueueWork(new JobSegment(state, execute, start, count));
            }
        }
    }

    private void EnqueueWork(in JobSegment segment)
    {
        var index = (int)(Interlocked.Increment(ref nextQueueIndex) % workerCount);
        queues[index].Queue.Enqueue(segment);
        try
        {
            workAvailable.Release();
        }
    ...
    private void WorkerLoop(int workerIndex)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            if (!TryGetWork(workerIndex, out var segment))
                continue;
    ...
    private bool TryGetWork(int workerIndex, out JobSegment segment)
    {
        if (queues[workerIndex].Queue.TryDequeue(out segment))
            return true;
        ...
        for (var i = 0; i < workerCount; i++)
        {
            ...
            if (queues[i].Queue.TryDequeue(out segment))
                return true;
        }
    ...
    {
        public ConcurrentQueue<JobSegment> Queue { get; } = new();
    }
