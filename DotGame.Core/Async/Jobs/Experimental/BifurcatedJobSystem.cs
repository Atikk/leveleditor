...
namespace DotGame.Core.Async.Jobs.Experimental;

public sealed class BifurcatedJobSystem : IJobSystem
{
    private readonly QueueDomain[] domains;
    ...
    public BifurcatedJobSystem(int workerCount)
    {
        if (workerCount <= 0)
            throw new ArgumentOutOfRangeException(nameof(workerCount));

        this.workerCount = workerCount;
        var mainBackground = new QueueDomain(JobAffinity.MainThread | JobAffinity.Background | JobAffinity.IO);
        domains = new[]
        {
            mainBackground,
            new QueueDomain(JobAffinity.Background),
            new QueueDomain(JobAffinity.Render)
        };

        workers = new Thread[workerCount];
        for (var i = 0; i < workerCount; i++)
        {
            workers[i] = new Thread(() => WorkerLoop(i))
            {
                IsBackground = true,
                Name = $"BF-Worker-{i}"
            };
    ...
    public JobHandle CombineDependencies(ReadOnlySpan<JobHandle> handles, JobFence? fence = null)
    {
        ...
        var handle = CreateState(new JobScheduleOptions("Combine", JobPriority.Low, JobAffinity.Any, allowInlineExecution: true), 0, out var state, fence);
    ...
    public JobStatistics GetStatistics()
    {
        ...
        var active = Math.Clamp(Volatile.Read(ref activeWorkers), 0, workerCount);
    ...
    private void QueueSegments(JobState state, JobExecuteDelegate execute, int iterationCount, JobScheduleOptions options)
    {
        ...
        var domain = SelectDomain(options);
        ...
        for (var segmentIndex = 0; segmentIndex < segments; segmentIndex++)
        {
            ...
            var segment = new JobSegment(state, execute, start, count);

            if (options.AllowInlineExecution && segmentIndex == 0 && segments == 1 && domain == domains[0])
            {
                ExecuteSegment(segment.State, segment.Execute, segment.Start, segment.Count, token, inline: true);
            }
            else
            {
                domain.Enqueue(segment);
                wakeSignal.Set();
            }
        }
    ...
    private QueueDomain SelectDomain(JobScheduleOptions options)
    {
        foreach (var domain in domains)
        {
            if (domain.Supports(options.Affinity))
                return domain;
        }
    ...
    private void WorkerLoop(int workerIndex)
    {
        ...
        {
            JobSegment segment;
            if (!TryDequeueWork(out segment))
            {
                wakeSignal.WaitOne(TimeSpan.FromMilliseconds(2));
                continue;
            }

            ExecuteSegment(segment.State, segment.Execute, segment.Start, segment.Count, token, inline: false);
        }
    }

    private bool TryDequeueWork(out JobSegment segment)
    {
        foreach (var domain in domains)
        {
            if (domain.TryDequeue(out segment))
                return true;
        }
    ...
    private void ExecuteSegment(JobState state, JobExecuteDelegate execute, int start, int count, CancellationToken token, bool inline)
    {
        ...
        try
        {
            for (var i = 0; i < count; i++)
            {
                ...
                var worker = inline ? -1 : Thread.CurrentThread.ManagedThreadId;
                var context = new JobExecutionContext(token, worker, iterationIndex, state.IterationCount);
                execute(context);
            }
    ...
    {
        if (disposed)
            throw new ObjectDisposedException(nameof(BifurcatedJobSystem));
    }

    private readonly struct JobSegment
    {
        public JobSegment(JobState state, JobExecuteDelegate execute, int start, int count)
        {
            State = state;
            Execute = execute;
            Start = start;
    ...
    private sealed class QueueDomain
    {
        private readonly ConcurrentQueue<JobSegment> queue = new();
        private readonly ManualResetEventSlim signal = new(false);

        public QueueDomain(JobAffinity affinity)
        {
            Affinity = affinity;
        }
        ...
        public bool Supports(JobAffinity affinity) => affinity == JobAffinity.Any || (Affinity & affinity) != 0;

        public void Enqueue(in JobSegment segment)
        {
            queue.Enqueue(segment);
            signal.Set();
        }

        public bool TryDequeue(out JobSegment segment)
        {
        ...
        public bool WaitForWork(int timeoutMilliseconds, CancellationToken token)
        {
            try
            {
                return signal.Wait(timeoutMilliseconds, token);
            }
            catch (OperationCanceledException)
            {
                return false;
